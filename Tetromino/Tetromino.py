...
def main():
    ...
    while True:
        if random.randint(0, 1) == 0:
            pygame.mixer.music.load('Portal2-24-Robots_FTW.mp3')
        else:
            pygame.mixer.music.load('Portal2-13-Want_You_Gone.mp3')
        pygame.mixer.music.play(-1, 0.0)
        runGame()
        pygame.mixer.music.stop()
        showTextScreen('Game Over')
#good
print('you ruin everything')
def runGame():
    ...
    print('Pieces created, starting loop')
        ...
        for event in pygame.event.get():
            if event.type == KEYUP:
                ...
                if(event.key == K_LEFT or event.key == K_a) and isValidPosition(board, fallingPiece, adjX=-1):
                    ...
                    movingRight = False
                    lastMoveSidewaysTime = time.time()
                    print('Shape moved left')
                elif (event.key == K_RIGHT or event.key == K_d) and isValidPosition(board, fallingPiece, adjX=1):
                    fallingPiece['x'] += 1
                    movingRight = True
                    movingLeft = False
                    lastMoveSidewaysTime = time.time()
                    print('Shape moved right')

                elif (event.key == K_UP or event.key == K_w):
                    fallingPiece['rotation'] = (fallingPiece['rotation'] + 1) % len(SHAPES[fallingPiece['shape']])
                    if not isValidPosition(board, fallingPiece):
                        fallingPiece['rotation'] = (fallingPiece['rotation'] - 1) % len(SHAPES[fallingPiece['shape']])
                    print('Shape rotated clockwise')
                elif (event.key == K_q):
                    fallingPiece['rotation'] = (fallingPiece['rotation'] - 1) % len(SHAPES[fallingPiece['shape']])
                    if not isValidPosition(board, fallingPiece):
                        fallingPiece['rotation'] = (fallingPiece['rotation'] + 1) % len(SHAPES[fallingPiece['shape']])
                    print('Shape rotated counter-clockwise')
...
def checkForQuit():
    for event in pygame.event.get(QUIT):
        print('Check for quit found pygame QUIT event')
        terminate()
    for event in pygame.event.get(KEYUP):
        if event.key == K_ESCAPE:
            print('Check for quit found user released the Esc key')
            terminate()
